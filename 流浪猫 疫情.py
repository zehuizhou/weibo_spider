...
class WbSpider:
    ...
    def get_web_data(self, retry_count):
        ...
        for div in div_list:
            ...
            if '素食助力' not in web_item['内容'] and '纯素助力' not in web_item['内容']:
...
if __name__ == '__main__':
    change_proxy(1)
    # 日期要多加1天
    date_list = ['2020-01-01', '2020-02-01', '2020-03-01', '2020-04-01', '2020-05-01', '2020-06-01']
    ...
    for key in key_list:
        for d in range(1, len(date_list)):
            ...
            wb = WbSpider(keyword=key, start_time=date_list[d-1], end_time=date_list[d], page=1)
            data = wb.start()
            ...
            for i in range(2, total_page + 1):
                wb = WbSpider(keyword=key, start_time=date_list[d-1], end_time=date_list[d], page=i)
                data = wb.start()
                save_to_csv(file_name=key + '.csv', list_dict=data)
                print('############################')
